...
export const mostrarCarrito = (array) => {
    ...
    for (let i = 0; i < array.length; i++) {
        seccionCarrito.appendChild(
            createCardAside(array[i].nombre, array[i].precioCalculado, array[i].cantidad)
        )
        console.log(array[i])
    }
}
...
export const mostrarLista = (array) => {
        ...
        for (let i = 0; i < array.length; i++) {
            seccionProductos.appendChild(
                createCard(array[i].nombre, array[i].precio, array[i].categoria)
            )
            console.log(array[i])
        }
    }
...
const createCard = (productName, price, category) => {
    ...
    //containers para el nuevo layout
    const container1 = document.createElement('div')
    container1.className = 'card__section'
    container1.append(h3, IconLetter)
    
    const container2 = document.createElement('div')
    container2.className = 'card__section'
    container2.append(info, priceT, buyBtn)
